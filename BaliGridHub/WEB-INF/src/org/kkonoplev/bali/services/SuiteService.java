...
public class SuiteService {
	...
	public GridSuiteExecContext getGridSuiteExecContext(String dir) throws IOException, ClassNotFoundException {
	
		// if in active suites
		GridSuiteExecContext contx = BaliServices.getGridHubService().getGridSuiteExecContext(dir);
		if (contx != null)
			return contx;
		
			
		log.info("Loading suite ExecContext from filesystem");
		String filepath = BaliProperties.getBaliResultsDir()+File.separator+dir+File.separator+GridSuiteExecContext.SUITE;
		contx = GridSuiteExecContext.deserialize(filepath);
		BaliServices.getGridHubService().getgsuiteExecContexts().add(contx);
	...
	public HashMap<String, String> getLastTenGridSuiteContexts() {
		
		File[] f = getHistory();

		HashMap<String, String> s = new LinkedHashMap<String, String>(30);
		
		for (GridSuiteExecContext execContext: BaliServices.getGridHubService().getgsuiteExecContexts()){

...
<script type="text/javascript">
 ...
 var grid = {
		 "nodeSuites":[
		                ...
		                {"nodeId":"${nodeSuite.node.id}", "url":"${nodeSuite.node.url}", "resultDir":"${nodeSuite.suiteMdl.resultDir}" },
		               </c:forEach>		
			   		   ]
 };
...
<body id="InfHtmlBody">
   ...
   <div class="clearBoth" style="padding-left:40px; padding-top:20px;">
   	   <input type="radio" checked onclick="refreshAllPage();" name="currentNode" value="0"> <b> All  &nbsp;</b>
       <c:forEach items="${gsuiteExecContext.gsuiteMdl.nodeSuites}" var="nodeSuite">
         <input type="radio" onclick="refreshNodePage('${nodeSuite.suiteMdl.resultDir}', '${nodeSuite.node.url}', '${nodeSuite.node.id}');" name="currentNode" value="${nodeSuite.node.id}">  <b> Node ${nodeSuite.node.id} &nbsp</b>	     
	   </c:forEach>		
	</div>
...
function renderGridSuiteHeader(){
	var items = [];
	items.push('<TABLE style="padding-left:40px; padding-top:20px;">');
	items.push('<tr>');
	items.push('	<td class="svcAll" style="width: 400px;"> ');
	items.push('		<a target="_blank" href="/baligridhub/form/status/classifyreport?resultDir=${gsuiteExecContext.resultDir}"> classify report </a> &nbsp&nbsp');
	items.push('		<a target="_blank" href="/baligridhub/form/status/capturereport?resultDir=${gsuiteExecContext.resultDir}"> screen capture report </a> &nbsp');
	items.push('		<a href="/baligridhub/form/status/perfomancereport?resultDir=${gsuiteExecContext.resultDir}"> metrics report </a> &nbsp');
	items.push('	</td>  ');
	return items.join('');
}
...
function refreshNodePage(resultDir,nodeUrl,nodeId){

  $.getJSON(nodeUrl+'/form/status/suitecontext/json?resultDir='+resultDir, 
  ...
  grid.nodeSuites.forEach(function(nodeSuite, i, arr) {
	  	$("#node"+nodeSuite.nodeId).html('');
  });
  
  clearTimeout(timeout); 
  timeout = window.setTimeout("refreshNodePage('"+resultDir+"','"+nodeUrl+"');", 5000);
...
function refreshAllPage(){
	...
	grid.nodeSuites.forEach(function(nodeSuite, i, arr) {
		$.getJSON(nodeSuite.url+'/form/status/suitecontext/json?resultDir='+nodeSuite.resultDir, 
				  
				  function(suiteExecContext) {
			  		var src = [];
			  		if (i==0) src.push(renderGridSuiteHeader());
			  		
			  		src.push('<TABLE style="padding-left:40px; padding-top:20px;">');
			  		src.push(renderSuiteTableHeader(suiteExecContext, nodeSuite.nodeId, nodeSuite.url));
			  		src.push(renderSuiteTable(suiteExecContext, nodeSuite.url, nodeSuite.resultDir));
			  		src.push('<TABLE>');
				  	$("#node"+nodeSuite.nodeId).html(src.join(''));
				 }

...
import org.kkonoplev.bali.suiteexec.mdl.ExecutorsInfo;
import org.kkonoplev.bali.suiteexec.mdl.TestExecContextInfo;


public class SuiteExecProcessor {

	private static final Logger log = Logger
			.getLogger(SuiteExecProcessor.class);
	public static int MAX_EXEC_SUITES = 20;

	private List<SuiteExecContext> suiteExecContexts = new ArrayList<SuiteExecContext>(
			MAX_EXEC_SUITES);
	...
	public SuiteExecProcessor(ProjectService projectSvc, int execCount)
			throws Exception {
		...
		if (CommunityEdition.isEnabled()
				&& execCount > CommunityEdition.MAX_THREADS)
	...
	public synchronized TestExecContext detachNextFreeJob() {
		...
		for (int i = 0; i < suiteExecContexts.size(); i++) {
			...
			SuiteExecContext suiteExecContext = suiteExecContexts
					.get((i + curPointer) % suiteExecContexts.size());
			TestExecContext testExecContext = suiteExecContext
					.getNextFreeJobWithPreLockedResources();
	...
	public synchronized void onSuiteExecFinished(SuiteExecContext execContext) {
		log.info("Suite " + execContext + " execution finished at "
				+ execContext.getEndDate() + ".");
		execContext.save();
	...
	private void sendMailNotify(SuiteExecContext execContext) {
		...
		if (!email.equals("")) {
			...
			String url = getBaseURL()
					+ "/form/status/suitecontextemail?resultDir="
					+ execContext.getResultDir();
			log.info("url=" + url);
			MailUtil.sendUrlHTML("BaliTestServer@db.com", email, "",
					"Bali Suite Execution Results " + desc, url, true);
		}
	...
	public synchronized SuiteExecContext addSuiteToJobQueue(SuiteMdl suiteMdl,
			String dir, String confDir, Map<String, String> environments)
			throws Exception {

		SuiteExecContext execContext = (new SuiteExecContextBuilder()).build(
				suiteMdl, this, dir, confDir, environments);
		suiteExecContexts.add(execContext);
	...
	public synchronized void onTestJobFinished(TestExecContext testExecContext) {
		...
		testExecContext.getSuiteExecContext()
				.onTestJobFinished(testExecContext);
	...
	public synchronized void rerunWarnings(SuiteExecContext suiteExecContext,
			String warningsIds) throws Exception {
		...
		ArrayList<String> rerunTestClasses = calcRerunTestClasses(
				suiteExecContext.getClassifyReport(), warningsIds);
		clearClassifyReportByWarnIDs(suiteExecContext.getClassifyReport(),
				warningsIds);
		clearClassifyReportFromTestClasses(
				suiteExecContext.getClassifyReport(), rerunTestClasses);
		clearClassifyReportByFirstTestClassMatch(
				suiteExecContext.getCaptureReport(), rerunTestClasses);
	...
	public synchronized void rerunTestExecContext(
			TestExecContext testExecContext) throws Exception {

		log.info("Rerun testExecContext " + testExecContext.toString());

		SuiteExecContext suiteExecContext = testExecContext
				.getSuiteExecContext();

		clearClassifyReportsFromTestContext(suiteExecContext, testExecContext);
		saveClassifyReports(suiteExecContext);

		// if suite is not active on proccessor, means was deserialized at the
		// moment
		if (findSuiteExecContext(suiteExecContext.getResultDir()) == null) {
			suiteExecContext.setExecProcessor(this);
	...
	private void resetTestExecContexts(SuiteExecContext suiteExecContext,
		...
		for (String className : rerunTestClasses) {
			...
			if (testExecContext != null)
				...
				log.error("Can't find testExecContext for class " + className
						+ " in suite " + suiteExecContext.getResultDir());
			}
		}
		log.info("Reset Test Exec Contexts done.");
	}

	private void clearClassifyReportByWarnIDs(ClassifyReport classifyReport,
			String warningsIds) {
	...
	private void clearClassifyReportFromTestClasses(
			ClassifyReport classifyReport, ArrayList<String> testClasses) {
	...
	private void clearClassifyReportByFirstTestClassMatch(
			ClassifyReport classifyReport, ArrayList<String> rerunTestClasses) {
	...
	public ArrayList<String> calcRerunTestClasses(
			ClassifyReport classifyReport, String warningsIds) {
	...
	private void clearClassifyReportsFromTestContext(
			SuiteExecContext suiteExecContext, TestExecContext testExecContext)
			throws Exception {
	...
	public void saveClassifyReports(SuiteExecContext suiteExecContext) {
		...
		ClassifyReportHTMLBuilder classifyReportBuilder = new ClassifyReportHTMLBuilder(
				suiteExecContext.getClassifyReport());
		classifyReportBuilder.save();

		classifyReportBuilder = new ClassifyReportHTMLBuilder(
				suiteExecContext.getCaptureReport());
		classifyReportBuilder.save();
	...
	public ExecutorsInfo getExecutorsInfo() {
		...
		int total = this.testExecutors.size();
		int busy = 0;

		for (TestExecutor texec : testExecutors)
			if (texec.getTestExecContext() != null)
				busy++;

		ExecutorsInfo execInfo = new ExecutorsInfo(total, busy);
		return execInfo;

	}

	public ArrayList<TestExecContextInfo> getTestExecutorsInfo() {
		ArrayList<TestExecContextInfo> testExecInfo = new ArrayList<TestExecContextInfo>();
		for (TestExecutor texec : testExecutors)
			testExecInfo.add(convert(texec));
		return testExecInfo;
	}

	private TestExecContextInfo convert(TestExecutor texec) {
		TestExecContextInfo ctxInfo = new TestExecContextInfo();

		ctxInfo.setExecutorName(texec.getName());
		ctxInfo.setExecutorState(texec.getState().name());
		if (texec.getTestExecContext() != null) {
			TestExecContext execctx = texec.getTestExecContext();
			ctxInfo.setLineViewLabel(execctx.getRunnableNode()
					.getLineViewLabel());
			ctxInfo.setClassName(execctx.getClassName());
			ctxInfo.setErrorCount(execctx.getErrorCount() + "");
			ctxInfo.setStatus(execctx.getStatus().getDesc());
			ctxInfo.setStartDate(execctx.getStartDateFmt());
			ctxInfo.setEndDate(execctx.getEndDateFmt());
			ctxInfo.setElapsed(execctx.getElapsedFmt());

			ctxInfo.setNeedStop(execctx.getNeedStop());
			ctxInfo.setDebugMode(execctx.getDebugMode());
			ctxInfo.setCaptureMode(execctx.getCaptureMode());
			ctxInfo.setCompleted(execctx.getCompleted());
			ctxInfo.setResourcesNames(execctx.getResourcesNames());
			ctxInfo.setThreadId(execctx.getThreadId() + "");

			ctxInfo.setResultDir(texec.getTestExecContext()
					.getSuiteExecContext().getResultDir());
		}
	...
	public int cleanHaltedThreads() {
		...
		while (it.hasNext()) {
			...
			if (exec.getState() != State.WAITING
					&& exec.getTestExecContext() != null
					&& exec.getTestExecContext().getTestExecutor() == null) {
				it.remove();
				cleaned++;
			}
		}
